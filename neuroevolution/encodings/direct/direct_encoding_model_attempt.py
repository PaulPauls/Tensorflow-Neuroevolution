...
class DirectEncodingModel(tf.keras.Model):
    ...
    def __init__(self, genotype, activations, trainable):
        ...
        tf.print("Running model eagerly improve debugging: ", self.run_eagerly)
    ...
    def call(self, inputs, **kwargs):
        ...
        inputs = tf.expand_dims(tf.cast(inputs, tf.float64), 0)
        for layer_functions in self.custom_layers:
            ...
            inputs = tf.concat([inputs, tf.expand_dims(layer_out, 0)], 0)
        return inputs[-1, :, :]
