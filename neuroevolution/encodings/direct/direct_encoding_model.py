...
class CustomWeightAndInputLayer(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, activation, kernel_weights, bias_weights, input_node_coords, trainable, dtype, dynamic):
        super(CustomWeightAndInputLayer, self).__init__(trainable=trainable, dtype=dtype, dynamic=dynamic)
        self.activation = activation
        self.kernel = self.add_weight(shape=kernel_weights.shape,
                                      dtype=self.dtype,
                                      initializer=CustomWeightAndInputLayer.initializer,
...
class DirectEncodingModel(tf.keras.Model):
    ...
    def __init__(self, genotype, trainable, dtype, run_eagerly):
        ...
        :param trainable: bool flag setting if model weights can be trained via regular Tensorflow infrastructure
        ...
        nodes, connections, node_dependencies = self._process_genotype(genotype)

        self.topology_levels = list(toposort(node_dependencies))
        node_coordinates = self._create_node_coordinates(self.topology_levels)
        ...
        for layer_index in range(len(self.custom_layers)):
            ...
            joined_layer_node_dependencies = self._join_keys(layer_node_dependencies)
            ...
            for joined_nodes, joined_nodes_input in joined_layer_node_dependencies.items():
                ...
                # in unison
                nodes_function = CustomWeightAndInputLayer(activation=activation,
    ...
    def _create_node_coordinates(topology_levels) -> dict:
        ...
        :param topology_levels: List with each element specifying the set of nodes that have to be precomputed before
    ...
    def _join_keys(node_dependencies) -> dict:
        ...
        dict value. Also convert the dict value of the input parameter to lists. Return this converted dict.
